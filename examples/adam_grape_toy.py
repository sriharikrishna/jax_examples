''' This is a toy problem to mimic quantum optimal control
    Initial state: x(0): 0
...
'''
    Cost function that penalizes control values above 20 and sums values below 20
'''
def cost(params, x, target):
  return target-sum(np.where(params <= 20.0, params, -params))+x

'''
    Step function gets the existing parameters, computes the gradient and reinvokes the
    optimization update
'''
@jit
...
'''
    Toy version of the grape algorithm
    After initialization, the loop repeatedly performs the step funtion till convergence
'''
def grape():
  step_count = 100
